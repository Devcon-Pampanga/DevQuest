...
import { SubquestStatusPill } from "@/components/quests/subquests/SubquestStatusPill";

export function SubquestTile({
  subquest,
  diffMeta,
  status,
  completion,
  ...
  status: SubquestCompletionStatus | "available";
  completion?: SubquestCompletion;
  isExpanded: boolean;
  ...
  const canJoin = !isCoordinator && status === "available" && subquest.assignmentType === "open";
  const canSubmit = !isCoordinator && (status === "joined" || status === "assigned");

  const deadlineStr = subquest.deadline
    ? subquest.deadline.toDate().toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })
    : null;
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 flex-wrap mt-1">
            ...
            <SubquestStatusPill status={status} />
        ...
        >
          ...
          <div className="flex flex-wrap gap-3 mt-3">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-text-muted capitalize">{subquest.assignmentType === "open" ? `Open · ${subquest.slots ?? "∞"} slots` : subquest.assignmentType}</span>
            </div>
          </div>
          ...
          {subquest.submissionGuidance && canSubmit && (
            <div className="mt-3 rounded-lg bg-[#ffffff06] border border-[#27272A] px-3 py-2">
              ...
              <p className="text-xs text-text-secondary">{subquest.submissionGuidance}</p>
            </div>
          )}
          ...
          {canJoin && (
            ...
            >
              {submitting ? "Joining…" : "Join Subquest"}
            </button>
          )}
            ...
            <div className="mt-3">
              ...
              {status === "completed" && (
                <p className="text-xs text-emerald-400 text-center py-2 font-heading">
                  ✓ Completed · +{subquest.xpReward} XP earned
                </p>
              )}
            </div>
          )}
